# sodachi-frontend-dist

そだちログの **OTA バンドル配信先** (GitHub Pages)。

- 中身は `sodachi-frontend` (private) の CI が自動生成して push する。**手で編集しない。**
- 配布物は Ed25519 で署名されており、アプリ側は署名検証に通ったバンドルしか適用しない。

Base URL: `https://pad01g.github.io/sodachi-frontend-dist/`
...
なお **こどもの記録はここには含まれない**。配っているのは画面と処理 (JS/CSS) だけで、
利用者のアカウントも記録も自宅サーバと端末の中にしかない。
